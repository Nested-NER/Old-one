...
class TOI_Pooling(nn.Module):
    def __init__(self, input_size, if_gpu, pooling_size, pooling_method = 'average'):
...
class TOI_BERT(nn.Module):
    ...
    def forward(self, mask_batch, word_batch, char_batch, char_len_batch, pos_batch, toi_batch, bert_hidden, entity_idx):
        ...
        if self.config.use_bert:
            ...
            if self.config.fusion_sum:
                if self.config.use_last_four:
                    ...
                    bert_hidden_out *= self.config.omega #0.3
            else:
        ...
        features, toi_section = self.hat_1(features.transpose(1, 2), toi_batch)
...
class DTE(nn.Module):
    ...
    def forward(self, mask_batch, word_batch, char_batch, char_len_batch, pos_batch):
